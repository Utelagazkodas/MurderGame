<script lang="ts">
  ...
  let {
    ...
    thisPlayer: player;
    killUser: Writable<player | undefined>;
    killPopUp: Writable<boolean>;
  } = $props();
  ...
  onMount(()=>{
    console.log(thisPlayer.nickname)
  })
</script>
...
>
  {#if thisPlayer.alive}
    ...
    <div class="flex justify-center">
      <span class="mr-2">{fallBack(thisPlayer.name, thisPlayer.nickname)}</span>
      ...
      {:else if $gameState && $gameState.player && isMeeting($gameState.gamedata) && $gameState.player.revealDeath == null}
        ...
        >
          <svg
            ...
            stroke-linecap="round"
            stroke-linejoin="round"
            class="lucide lucide-vote text-{thisPlayer.publicID == $votedForId
              ? 'green-800'
              : 'green-300'}"
    ...
    <div class="text-gray-700 text-center w-full flex justify-center text-base">
      {#if isMeeting($gameState.gamedata)}
        {#each $gameState.players as player}
          ...
          {/if}
        {/each}
      {:else if thisPlayer.canCallMeeting > 0}
        ...
        <svg
          ...
          stroke-linecap="round"
          stroke-linejoin="round"
          class="lucide lucide-megaphone -rotate-12"
      ...
      {:else}
        <svg
          ...
          stroke-linecap="round"
          stroke-linejoin="round"
          class="lucide lucide-megaphone-off -rotate-12"
          ><path d="M9.26 9.26 3 11v3l14.14 3.14" /><path
            d="M21 15.34V6l-7.31 2.03"
          /><path d="M11.6 16.8a3 3 0 1 1-5.8-1.6" /><line
            x1="2"
            x2="22"
            y1="2"
            y2="22"
  ...
  {:else}
    ...
    {#if $gameState.gamedata.killerID && $gameState.gamedata.killerID == thisPlayer.publicID}
      <div class="line-through">Gyilkos</div>
    {:else}
      <div class="line-through">Ártatlan</div>
    {/if}
  {/if}
</div>
